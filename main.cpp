...
TCHAR		szMainWindowClassName[] = _T("WindowFinderMainWindow"); // Name of the Main Window class.
HINSTANCE	g_hInst = NULL;
HWND		g_hwndMainWnd = NULL;
HANDLE		g_hApplicationMutex = NULL;
DWORD		g_dwLastError = 0;
BOOL		g_bStartSearchWindow = FALSE;
BOOL		g_bHideSearchWindow = FALSE;
...
HWND		g_hwndTreeControl = NULL;
HGDIOBJ		g_hPrevPen = NULL;
HGDIOBJ		g_hPrevBrush = NULL;
...
BOOL InitialiseResources()
{
  ...
  g_hRectanglePen = CreatePen (PS_SOLID, 6, RGB(0, 0, 256));
...
)
{
  ...
  bRet = InitializeApplication (hInstance, hPrevInstance, (LPTSTR)lpCmdLine, nCmdShow);
  if (bRet == FALSE)
  {
  ...
  StartSearchWindowDialog (g_hwndMainWnd);
  ...
  switch(message) 
  {
	case WM_COMMAND:
	{
	  ...
	  if (wNotifyCode == 0)
	  {
		...
		if (wID == IDM_ABOUT)
		{
		  MessageBox (NULL, ABOUT_WINDOW_FINDER, _T("Window Finder"), MB_OK);
		}
